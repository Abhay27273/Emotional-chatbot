...
logger.info("--- SIMPLE STT-LLM-TTS VERSION ---")
...
# --- SYSTEM PROMPT ---
SYSTEM_PROMPT = """You are a helpful and friendly AI assistant on a phone call.
Be conversational and concise. Keep responses brief and natural."""
...
async def twilio_ws(websocket: WebSocket):
    """Handles the WebSocket connection from Twilio, running the Pipecat pipeline."""
    logger.info("=" * 80)
    ...
    try:
        ...
        transport = FastAPIWebsocketTransport(
            ...
            params=FastAPIWebsocketParams(
                ...
                vad_enabled=False,  # VAD disabled
        ...
        # --- Simple pipeline: transport -> stt -> llm -> tts -> transport ---
        pipeline = Pipeline([
            transport.input(),
            stt,
        ...
        await task.queue_frame(TextFrame("Hello! I'm your AI assistant. How can I help you today?"))

        logger.info("Pipeline running, waiting for completion...")
        await pipeline_task
